...
def segment(path):
    ...
    equ = ""
    inputs = []
    equ = get_equ(gates , output,inputs)
    print(output, "=", equ)
    equ = output, "=", equ
    return equ, array_line, final_image, output , inputs
...
def get_equ( gates , output ,inputs):
    ...
    if index == len(gates) + 1 :
      ...
      equ  = equ +" ( "+ get_equ( gates , gates[index][2],inputs)
      ...
      equ = equ + get_equ( gates , gates[index][3],inputs) + " ) "
